...
router.get('/pasing', function(req, res){
...
fs.readFile('list.html', 'utf-8', function (error, data) {
  ...
  getConnection().query('delete from topic where id = ?', [req.params.id], function () {
  res.redirect('/main')
  });
  ...
  var body = req.body;
  getConnection().query('insert into topic(id,title,created) values (?,?,?)', [body.name, body.num, body.section], function () {
  //응답
  res.redirect('/main');
  })
  ...
  getConnection().query('select * from topic where id = ?', [req.params.id], function (error, result) {
  res.send(ejs.render(data, {
  data: result[0]
  }))
  })
  ...
  var body = req.body;
  getConnection().query('update topic set title = ?, description = ?, created = ? where id = ?',
